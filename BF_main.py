...
filt_freq = 15000
mic_data1 = "/media/xian/Data/dataset/Bekaert/h5files/41.h5"
mic_data2 = "/media/xian/Data/dataset/Bekaert/h5files/42.h5"
mic_data3 = "/media/xian/Data/dataset/Bekaert/h5files/43.h5"


ts = acoular.MaskedTimeSamples( name=mic_data1, invalid_channels=[9] )
...
# 3 lm
ts = acoular.MaskedTimeSamples( name=mic_data3, invalid_channels=[9] )
ps = acoular.PowerSpectra( time_data=ts, block_size=1024, window="Hanning" )
bb = acoular.BeamformerMusic( freq_data=ps, steer=st)
bb.n = 5
...
for res, ax in zip(results, axes.flat):
    im = ax.imshow( res.T, origin="lower", vmin=Lm.max()-15, extent=rg.extend() )
